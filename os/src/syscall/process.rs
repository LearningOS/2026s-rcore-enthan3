...
pub fn sys_get_time(ts: *mut TimeVal, _tz: usize) -> isize {
    ...
    let mut dst = translated_byte_buffer(token, ts as *const u8, core::mem::size_of::<TimeVal>());
    let mut offset = 0usize;
    for bytes in dst.iter_mut() {
        let len = bytes.len();
        bytes.copy_from_slice(&src[offset..offset + len]);
        offset += len;
    }
    0
}
...
pub fn sys_munmap(start: usize, len: usize) -> isize {
    ...
    if inner.memory_set.remove_area_range(start_va, end_va) {
        0
    } else {
        -1
    }
